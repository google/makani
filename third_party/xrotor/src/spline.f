...
ccc      SS2 = S2(1)
      ...
      DS2 = 0.0
C
      EPS1 = EPS*ABS(S1(N1)-S1(1))
      EPS2 = EPS*ABS(S2(N2)-S2(1))
C
      DO 1000 ITER=1, 12
C
        RLX = 1.0
...
C
          ...
          RS2 = YY1 - YY2
C
          IF(ABS(RS1).LE.RS1OLD+EPS1 .AND.
     &       ABS(RS2).LE.RS2OLD+EPS2      ) GO TO 11
C
          RLX = 0.5*RLX
C
 10     CONTINUE
...
C
        IF(ABS(DS1) .LT. EPS1 .AND.
     &     ABS(DS2) .LT. EPS2       ) RETURN
C
 1000 CONTINUE
...
C
      ...
      FUNCTION SSEVLIN(SS,X,S,N)
      DIMENSION X(N),S(N)
C------------------------------------------------------------
C     Calculates X(SS) using piecewise-linear                |
C     interpolation. This is intended for intepolating very  |
C     noisy data for which a cubic spline is inappropriate.  |
C------------------------------------------------------------
      ILOW = 1
...
C
      ...
      FUNCTION DSEVLIN(SS,X,S,N)
      DIMENSION X(N),S(N)
C------------------------------------------------------------
C     Calculates dX/ds(SS) using piecewise-linear            |
C     interpolation. This is intended for intepolating very  |
C     noisy data for which a cubic spline is inappropriate.  |
C------------------------------------------------------------
      ILOW = 1
