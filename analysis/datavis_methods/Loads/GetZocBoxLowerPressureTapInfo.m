...
function [xc_location, pressure_val] = GetZocBoxLowerPressureTapInfo(h5file_loc, zoc_id, unit)
  % GetZocBoxLowerPressureInfo collects wing lower surface ZocBox pressure tap location and pressure data
  %
  ...
  for ii = 1:no_ports;
    % check if port is on lower surface
    port_id = regexp(data_info.Groups(ii).Name, 'L\d{2,2}','match');

    if ~isempty(port_id);
      % locations
      h5_lowerloc = h5read(h5file_loc, [data_info.Groups(ii).Name, '/loc_xc']);
      xc_location = [xc_location; h5_lowerloc];

      % pressures, as measured in psi
      h5_lowerpress_psi = h5read(h5file_loc, [data_info.Groups(ii).Name, '/value']);
      pressure_val = [pressure_val; h5_lowerpress_psi];
    end
  end
