...
const isSupportedEnvironment = once(() =>
  (
    (typeof window !== 'undefined'
      && window !== null ? window.XMLHttpRequest : void 0)
    || (typeof window !== 'undefined'
      && window !== null ? window.XDomainRequest : void 0)
  ) && supportsInlineSVG()
);
...
const uniquifyIDs = (() => {
  ...
  const idPattern = new RegExp(
    `(?:(${(mkAttributePattern('id'))})="([^"]+)")|`
    + `(?:(${(mkAttributePattern('href'))}|${(mkAttributePattern('role'))}|`
  ...
  return (svgText, svgID) => {
    ...
    return svgText.replace(idPattern,
      (m, p1, p2, p3, p4, p5) => { //eslint-disable-line consistent-return
        if (p2) {
...
const getHash = (str) => {
  ...
  for (i = 0, j = 0, len = str.length;
       len <= 0 ? j < len : j > len; i = len <= 0 ? ++j : --j) {
...
export default class InlineSVG extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.src !== nextProps.src) {
      this.setState({status: Status.PENDING, loadedText: null});
  ...
  load(src) {
    ...
    return http.get(src, this.handleLoad);
  }
