...
*   gwxcursc   - gets current graphics cursor position whle button is down
...
/* gwxcursc
   Get current cursor position and key pressed while button is down 
   Parameters:  
     x           (int*)   output cursor x coordinate
     y           (int*)   output cursor y coordinate
     btn         (int*)   button status (1 for down, 0 for release)
...
/*  change event mask for mouse motion and button release */
   event_mask = ExposureMask | ConfigureNotify | 
                ButtonReleaseMask | ButtonMotionMask;
