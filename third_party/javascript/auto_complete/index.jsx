...
class AutoComplete extends Component {
  static propTypes = {
     ...
     * @param {string} key `dataSource` element, or `text` property on that
     *                     element if it's not a string.
     * @returns {boolean} `true` indicates the auto complete list will
     *                           include `key` when the input is `searchText`.
     */
    ...
    /**
     * Callback function that is fired when the `TextField` receives a keydown
     * event.
     */
     ...
     *
     * @param {string} chosenRequest Either the `TextField` input value, if
     *                               enter is pressed in the `TextField`,
     *                               or the text value of the corresponding
     *                               list item that was selected.
     * @param {number} index The index in `dataSource` of the list item
     *                       selected, or `-1` if enter is pressed in the
     *                       `TextField`.
     */
    ...
    triggerUpdateOnFocus:
      deprecated(PropTypes.bool, 'Instead, use openOnFocus'),
  };
  ...
  handleItemTouchTap = (event, child) => {
    ...
    const searchText =
      typeof chosenRequest === 'string' ? chosenRequest : chosenRequest.text;
    const prevSearchText = this.props.searchText;

    this.props.onNewRequest(chosenRequest, index);

    this.timerTouchTapCloseId = setTimeout(() => {
      // Replace 'searchText' only if there were no changes since
  ...
  handleFocus = (event) => {
    if (!this.state.open
        && (this.props.triggerUpdateOnFocus || this.props.openOnFocus)) {
      this.setState({
        open: true,
        anchorEl: ReactDOM.findDOMNode(this.refs.searchTextField),
      });
        ...
        case 'object':
          if (item && typeof item.text === 'string') {
            if (this.props.filter(searchText, item.text, item)) {
              if (item.value.type
                && (item.value.type.muiName === MenuItem.muiName
...
AutoComplete.defaultFilter =
  AutoComplete.caseSensitiveFilter = (searchText, key) => {
    return searchText !== '' && key.indexOf(searchText) !== -1;
  };
...
AutoComplete.levenshteinDistanceFilter = (distanceLessThan) => {
  if (distanceLessThan === undefined) {
    ...
    throw 'Error: AutoComplete.levenshteinDistanceFilter is a filter generator'
      + ', not a filter!';
  }
