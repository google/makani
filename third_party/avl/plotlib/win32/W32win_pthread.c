    ...
    case WM_PAINT:
	 printf("WM_PAINT\n"); 
         ...
         */
         ...
         printf("hdc %d  hWnd %d  gwxmemHDC %d\n",hdc,hWnd,gwxmemHDC);
         if (gwxmemHDC != NULL) {
          printf("gwxmemHDC %d\n",gwxmemHDC);
...
/* gwxopen
   Open window display and get size and depth of root window
   Parameters:  
...
//    int  lunit; 
    ...
    /*
     Window sizing...
     Make window large enough to hold client area + borders for sides and top
    */
...
/* Must use WS_CLIPCHILDREN and WS_CLIPSIBLINGS styles. */
    ...
    SetWindowPos(gwxWin, HWND_TOP, WinRect.left, WinRect.top,
                                   WinRect.right, WinRect.bottom, 
                         SWP_NOMOVE|SWP_NOZORDER);
   
    GetClientRect(gwxWin, &rect);
    printf("\nThread1 client rect left %d right %d \n",rect.left,rect.right);
...
GWXWINOPEN (int *xstart, int *ystart, int *xsize, int *ysize)
{
  ...
  */  
    ...
    wysize   = *ysize;
    printf("\n In GWXWINOPEN\n");
    printf("wxstart %d wystart %d \n",wxstart,wystart);
    printf("wxsize  %d wysize  %d \n",wxsize,wysize);

    /* Start the window thread */
    initthread = 0;
    /*    printf("GWXOPEN calling pthread_create\n"); */
    ithread1 = pthread_create( &thread1, NULL, (void *) &Thread1, NULL);
    ...
    printf("initthread wait count %d\n",i);
          ...
          
{
    ...
    printf("\ngwxstatus window rect left %d right %d \n",rect.left,rect.right);
    printf("                        top  %d bot   %d \n",rect.top,rect.bottom);
    /*
    GetClientRect(gwxWin, &rect);
    printf("\ngwxstatus client rect left %d right %d \n",rect.left,rect.right);
    printf("                       top  %d bot   %d \n",rect.top,rect.bottom);
    */
    *xstart = rect.left;
    *ystart = rect.top;
    ...
    *ysize = rect.bottom - rect.top;
    /* add 1 to y size to account for last vertical client rectangle pixel 
     *ysize = *ysize + 1; */
    
    *xsize = *xsize - 2*gwxXmatte;
    *ysize = *ysize - 2*gwxYmatte - gwxYbar; 
        
    printf("\ngwxstatus window rect xstart %d ystart %d \n",*xstart,*ystart);
...
/* Immediate mode cursor motion query, returns cursor position while mouse is
   moving while button is held down, state flag gives button status 
   (1 for button pressed, 0 for release)  
*/
void W32CALL
