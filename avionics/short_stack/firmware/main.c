...
#include "avionics/short_stack/firmware/selftest.h"
...
#define STACKING_PERIOD_CYCLES CLOCK32_MSEC_TO_CYCLES(1)
...
int main(void) {
  ...
  while (true) {
    ...
    } else if (CLOCK32_GE(now, wakeup_stacking)) {
      wakeup_stacking += STACKING_PERIOD_CYCLES;
      ShortStackOutputSendStackingMessage();
    } else {
      NetPoll();
      NetMonPoll();
      NetDiagPoll();
